...
"$api_note_preamble This data was cached on $printable_cache_time";
    ...
    unless ( $raw_json and $decoded_json ) {
        if ( $options->{cache} ne 'never' ) {
            if (
                $c2j_cache->exists_and_is_expired(
            ...
            foreach my $type ( @{ $item->{'@type'} } ) {
                if ( $type eq 'vivo:Authorship' ) {    # pubs
                        ...
                        and $item->{'linkedInformationResource'} )
                    {
                        ...
                        push
                          @{ $publications_by_author{ $item->{'linkedAuthor'} }
                          },
...
"Tried to look up user specified, but got no results in the underlying data source. You can manually verify whether or not this is a valid Profiles user by visiting $canonical_url -- if you see a 404, the user's not in the system\n";
      ...
      )
    {
            ...
            m{$current_or_legacy_profiles_root_url_regexp/?profile/(\d+)$} )
        {
            ...
            unless ($raw_json_for_field) {
                if (
                    $url_cache->exists_and_is_expired(
            ...
            if ($raw_json_for_field) {
                    ...
                    and eval { $field_data->{entry}->{jsonld}->{'@graph'} } )
                {
                        ...
                        @{ $field_data->{entry}->{jsonld}->{'@graph'} } )
                    {
                        ...
                        if (
                            defined $item->{'applicationInstanceDataValue'}
...
# https://github.com/CTSIatUCSF/shindigorng/blob/master/src/main/java/edu/ucsf/orng/shindig/spi/OrngAppDataService.java
        ...
        for my $i ( 0 .. 199 ) {
            ...
            if ( $pub and ref $pub and ref $pub eq 'HASH' ) {
                    ...
                    and $pub->{'title'} =~ m{<span class="label">(.*?)</span>} )
                {
                    ...
                    my $maybe_title = $1;
                    if (    $maybe_title
                        and $maybe_title =~ m/[[:alpha:]]{3}.*[[:alpha:]]{3}/ )
                    {
                      ...
                      EachPubToScanTitles:
                            ...
                            @{ $publications_by_author{ $person->{'@id'} } } )
                        {
                            my $candidate_pub_id = $candidate_pub_data->{id};
                            my $label =
                              $items_by_url_id{$candidate_pub_id}->{'label'};
                            if (    $label
                                and $maybe_title
                                and $label eq $maybe_title )
                            {
                                push @pub_ids_that_match_this_title,
                                  $candidate_pub_id;
                            }
                        }
                        if ( @pub_ids_that_match_this_title == 1 ) {
                            $featured_publication_order_by_id{
                                $pub_ids_that_match_this_title[0] } =
                              $featured_num;
                            $matched_the_publication = 1;
                        }
                    }
    ...
    if ( $person->{'mailingAddress'} ) {
        ...
        if ( grep { m/Varies(?:, CA)? 0000[01]|^Varies$|Flexible, \#1/ }
            @address )
        {
            @address = ();
    ...
    if ( !defined $person->{'email'} and $person->{'emailEncrypted'} ) {
        ...
        state $config = eval {
            ...
            my $result          = eval {
                Config::Auto::parse( $config_filename, format => 'equal' );
            };
            return ( $result || {} );
        };
        ...
        if ( $config and eval { $config->{RC4_PASSWORD}->[0] } ) {
            ...
            eval {
                my $encrypted_bytes =
                  decode_base64( $person->{'emailEncrypted'} );
        ...
        Profiles => [
            {
                ...
                PublicationCount => eval {
                    if ( $publications_by_author{ $person->{'@id'} } ) {
                        return
                          scalar @{ $publications_by_author{ $person->{'@id'} }
                          };
                    }
                    else {
                        return 0;
                ...
                Videos => (
                    eval {
                        ...
                        for my $video_field ( 'UCSFFeaturedVideos',
                            'FeaturedVideos' )
                        {
                            next if @raw_videos_array;
                            next unless $person->{$video_field};
                            my $item =
                              $items_by_url_id{ $person->{$video_field} };
                            next unless $item and $item->{'pluginData'};
                            my $pd = $item->{'pluginData'};

                            # scalar JSON string → decode directly
                            if ( !ref $pd ) {
                                my $decoded = eval {
                                    decode_json( Encode::encode_utf8($pd) );
                                };
                                if ( $decoded and ref $decoded eq 'ARRAY' ) {
                                    @raw_videos_array = @{$decoded};
                                }

                       # array of JSON-encoded strings (upstream format change):
                       # deduplicate strings, decode each, flatten results
                            }
                            elsif ( ref $pd eq 'ARRAY'
                                and @$pd
                                and !ref $pd->[0] )
                            {
                                my %seen;
                                ...
                                my %seen_url;
                                @raw_videos_array = grep {
                                    !$seen_url{ $_->{url} // $_->{youTubeId}
                                          // $_ }++
                                } @raw_videos_array;

                                # already a decoded array-of-hashes
                            }
                            elsif ( ref $pd eq 'ARRAY' ) {
                                @raw_videos_array = @{$pd};
                            }
                ...
                FacultyMentoring => (
                    eval {
                            ...
                            and $items_by_url_id{ $person->{'Mentoring'} } )
                        {
                            ...
                            my $plugin_node =
                              $items_by_url_id{ $person->{'Mentoring'} };
                            my @maybe_json =
                            ...
                            for my $json_string (@maybe_json) {
                                ...
                                if ( $data->{'mentoringInterests'}
                                    and ref $data->{'mentoringInterests'} eq
                                    'ARRAY' )
                                {
                                    for my $mi (
                                        @{ $data->{'mentoringInterests'} } )
                                    {
                ...
                CollaborationInterests => (
                    eval {
                          ...
                          )
                        {
                            ...
                            my $plugin_node = $items_by_url_id{ $person->{
                                    'CollaborationInterests'} };
                            my @maybe_json =
                              ref $plugin_node->{'pluginData'} eq 'ARRAY'
                              ? @{ $plugin_node->{'pluginData'} }
                            ...
                            for my $json_string (@maybe_json) {
                                ...
                                if ( $data->{'collaborationInterests'}
                                    and ref $data->{'collaborationInterests'}
                                    eq 'ARRAY' )
                                {
                                    for my $item (
                                        @{ $data->{'collaborationInterests'} } )
                                    {
                                        next unless $item and $item =~ m/\w/;
                                        $interests->{Details}->{$item} =
                                          JSON::true;
