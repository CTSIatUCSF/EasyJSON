...
my $api = new ProfilesEasyJSON::ClassicUCSF;
...
SKIP: {
        ...
        cmp_ok(
            eval {
                scalar( @{ $data->{Profiles}->[0]->{GlobalHealth_beta}->{Countries} } );
            },
            '>=',
            1,
...
SKIP: {
        ...
        is_deeply( $data->{Profiles}->[0]->{Address}->{Latitude},
            undef, "$test_name: Latitude is undef as expected [regression]" );
    }
}
...
SKIP: {
        ...
        cmp_ok( scalar( @{ $data->{Profiles}->[0]->{ResearchActivitiesAndFunding} } ),
            '<=', 4, "$test_name: No more than 4 grants" );
...
SKIP: {
        ...
        like(
            eval { scalar $data->{Profiles}->[0]->{WebLinks_beta}->[0]->{URL}; } // '',
